...
from matlab_like_parser.core.tokenizer import Token, TokenType
from matlab_like_parser.core.tree import *

from matlab_like_parser.core.parser_error import TreeConversionError
...
def convert_lark_tree(tree) -> Node:
    ...
    if isinstance(tree, LarkToken):
        if tree.type == 'NUMBER':
            return Number(Token(TokenType.NUMBER, tree.value, tree.line, tree.column))
        elif tree.type == 'IMAG_NUM':
            return ImagNumber(Token(TokenType.IMAG_NUMBER, tree.value, tree.line, tree.column))
        elif tree.type == 'IDENTIFIER':
            return Identifier(Token(TokenType.IDENTIFIER, tree.value, tree.line, tree.column))
        elif tree.type == 'TILDE':
            return Ignore(Token(TokenType.OPERATOR, tree.value, tree.line, tree.column))
        elif tree.type in ('CHAR', 'STR'):
            return String(Token(TokenType.STRING, tree.value, tree.line, tree.column))
        elif tree.type == 'COLON':
            return COLON(Token(TokenType.COLON, tree.value, tree.line, tree.column))
        # elif tree.type == 'WS':
        #     return WhiteSpace(Token(TokenType.SPACE, tree.value, tree.line, tree.column))
        else:
            # If we encounter an unhandled node type, raise an error with details
            raise TreeConversionError(f"Unhandled terminal in lark grammar file: {tree.type}", tree.line, tree.column)
    ...
    if tree.data == 'start':
        ...
        return convert_lark_tree(tree.children[0])
        ...
        return BinaryOperation(
            convert_lark_tree(tree.children[0]),  # Left operand
            tree.data,                            # Operator
            convert_lark_tree(tree.children[1])   # Right operand
        )
        ...
        if start_tree.data == 'colon_operator':
            start = convert_lark_tree(tree.children[0].children[0])
            step  = convert_lark_tree(tree.children[0].children[1])
        else:
        ...
        return UnaryOperation(
            tree.data,                            # Operator
            convert_lark_tree(tree.children[0])   # Operand
        )
        ...
        # Numeric literals
        return Number(Token(TokenType.NUMBER, tree.children[0].value, tree.children[0].line, tree.children[0].column))

    elif tree.data == 'char_array' or tree.data == 'string_array':
        # Character arrays and string literals
        return String(Token(TokenType.STRING, tree.children[0].value, tree.children[0].line, tree.children[0].column))
        ...
        if input_params:
            input_params = [convert_lark_tree(input_param) for input_param in input_params.children]
        else:
            input_params = []

        return AnonymousFunction(
            input_params,                               # Parameters
            [convert_lark_tree(tree.children[1])]       # Function body
        )

    elif tree.data == 'simple_identifier':
        # Simple identifiers (variable names)
        return Identifier(Token(TokenType.IDENTIFIER, tree.children[0].value, tree.children[0].line, tree.children[0].column))
        ...
        return IndexExpression(
            [convert_lark_tree(index) for index in tree.children]   # Indices
        )

    elif tree.data == 'struct_element':
        # Structure field access
        base = convert_lark_tree(tree.children[0])
        field_name = [convert_lark_tree(field) for field in tree.children[1:]]
        ...
        if len(tree.children) == 0 or tree.children[0] is None:
            ...
            return MatrixExpression(
                [
                    [convert_lark_tree(elem) for elem in row.children]
