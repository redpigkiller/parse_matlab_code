...
%% save directory
dir_dump = fullfile('_exp_dump_2', dir_sim);
...
%% declare record var.
r_avg_ber_Mc_n = zeros(n_Q, n_snr, Mc);
r_avg_ber_Mc_ml = zeros(n_Q, n_snr, Mc);
...
% for (i_Mc = 1 : Mc)
	...
	if sim_mode >= 2
		if isfile(path_full)
            ...
            %	get snr
            [load_snr, unmatched_snr] = intersects(default_sim_snr, data.snr);
            load_snr_idx = cmp_arr(load_snr, data.snr, 1);
            
            %	get Q
            [load_Q, unmatched_Q] = intersects(default_sim_Q, data.Q);
            load_Q_idx = cmp_arr(load_Q, data.Q, 1);
            
            unmatched = unmatched_snr || unmatched_Q;

            % record
            if unmatched
                r_avg_ber_Mc_n(:, :, i_Mc) = r_avg_ber(load_Q_idx, load_snr_idx);
            else
                r_avg_ber_Mc_n(:, :, i_Mc) = r_avg_ber;
            end			
		else
			valid_Mc(i_Mc) = 0;
		end
    ...
    elseif sim_mode >= 1 && isfile(path_full)
		...
		% run simulation
		[r_ber_Mc_n, r_ber_Mc_ml] = exp_one_Mc_ber_2(i_Mc, psimset.Value);

		% record
		r_avg_ber_Mc_n(:, :, i_Mc) = r_ber_Mc_n;
        r_avg_ber_Mc_ml(:, :, i_Mc) = r_ber_Mc_ml;

		% dump record
		parsave(path_full, r_ber_Mc_n, r_ber_Mc_ml, Q, snr);
	end
...
avg_ber_n = squeeze(sum(r_avg_ber_Mc_n, 3) ./ Mc);
avg_ber_ml = squeeze(sum(r_avg_ber_Mc_ml, 3) ./ Mc);
