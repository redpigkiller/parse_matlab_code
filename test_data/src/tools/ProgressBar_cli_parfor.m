...
%   char_set{1}↓↓char_set{4}    ↓char_set{2}    ↓char_set{3}  ↓char_set{5}
% downloading.. |██████████████████████████████               | 80%
% downloading.. |██████████████████████████████---------------| 80%
% downloading.. |██████████████████████████████•••••••••••••••| 80%
% downloading.. |==============================•••••••••••••••| 80%
...
bar_max = obj.terminal_width -4 -length(obj.char_set{4})-length(obj.char_set{5});
...
elseif obj.is_terminated || obj.int_percent == 100
    time_info = "Elapsed time: " ...
        + duration2string(datetime() - obj.start_time);
else
...
bar_bound_string_front = string(obj.char_set{4});
bar_bound_string_back = string(obj.char_set{5});
% percent_string = string(sprintf('%3d%%', obj.int_percent));
percent_string = string(sprintf('% 6.3f%%', 100*obj.ratio));

if obj.ratio > 1.0
    % exceed
    end_flag = false;
    % Bar text
    bar_string = [repmat(obj.char_set{6},[1,obj.bar_n])];
    second_line = obj.time_info ...
        + sprintf(" | number of finished tasks: %d (unknown total number of tasks)", obj.counter) ...
        + newline;

elseif obj.int_percent == 100 || obj.is_terminated
    % finished
    end_flag = true;
    % Bar text
...
first_line = ...
    name_string ...
    + bar_bound_string_front ...
    + bar_string ...
    + bar_bound_string_back ...
    + percent_string ...
    + newline;
